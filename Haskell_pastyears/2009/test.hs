import Data.List
import GHC.Data.Maybe (fromJust, fromMaybe)

type Index = Int

data BExp = Prim Bool | IdRef Index | Not BExp | And BExp BExp | Or BExp BExp
            deriving (Eq, Ord, Show)

type Env = [(Index, Bool)]

type NodeId = Int

type BDDNode =  (NodeId, (Index, NodeId, NodeId))

type BDD = (NodeId, [BDDNode])

------------------------------------------------------
-- PART I

-- Pre: The item is in the given table
lookUp :: Eq a => a -> [(a, b)] -> b
lookUp a x 
  = fromJust (lookup a x)



checkSat:: BDD -> Env -> Bool
checkSat (1, _) _ = True 
checkSat (0, _) _ = False
checkSat (curIndex, subtree)  env
 =  checkSat (nextIndex, subtree) env
    where (i,l,r) = lookUp curIndex subtree
          truth = lookUp i env
          nextIndex = if truth then r else l


sat :: BDD -> [[(Index, Bool)]]
sat = flip sat' []

sat' :: BDD -> [(Index,Bool)] -> [[(Index,Bool)]]
sat' (0,_) _ = []
sat' (1,_) env = [env]
sat' (i, subtree) env = sat' (left, subtree) (env ++ [(nextIndex, False)])
                         ++ sat' (right, subtree) (env++ [(nextIndex, True)])
                        where (nextIndex, left, right) = lookUp i subtree 

------------------------------------------------------
-- PART II

simplify :: BExp -> BExp
simplify (Not (Prim a)) = Prim (not a)
simplify (Or (Prim a) (Prim b)) = Prim (a||b)
simplify (And (Prim a) (Prim b)) = Prim (a&&b) 
simplify exp = exp 

restrict :: BExp -> Index -> Bool -> BExp
restrict (Prim x) _ _
  = Prim x
restrict (IdRef x) id bool
  | x == id   = Prim bool
  | otherwise = IdRef x
restrict (Not x) id bool
  = simplify (Not (restrict x id bool))
restrict (And x y) id bool
  = simplify (And (restrict x id bool) (restrict y id bool))
restrict (Or x y) id bool
  = simplify (Or (restrict x id bool) (restrict y id bool))
------------------------------------------------------
-- PART III

-- Pre: Each variable index in the BExp appears exactly once
--     in the Index list; there are no other elements
-- The question suggests the following definition (in terms of buildBDD')
-- but you are free to implement the function differently if you wish.
buildBDD :: BExp -> [Index] -> BDD
buildBDD e = buildBDD' e 2

-- Potential helper function for buildBDD which you are free
-- to define/modify/ignore/delete/embed as you see fit.
buildBDD' :: BExp -> NodeId -> [Index] -> BDD
buildBDD' e _ [] 
 | e == Prim True = (1,[])
 | e == Prim False = (0,[])
 | otherwise = (0,[])
buildBDD' e n (x:xs)
 = (n,[pair]++leftsubs++rightsubs) 
  where pair = (n,(x,l,r))
        left = restrict e x False 
        right = restrict e x True
        (l,leftsubs) = buildBDD' left (2*n) xs
        (r,rightsubs) = buildBDD' right (2*n+1) xs



------------------------------------------------------
-- PART IV

-- Pre: Each variable index in the BExp appears exactly once
--      in the Index list; there are no other elements
buildROBDD :: BExp -> [Index] -> BDD
buildROBDD e index 
  = undefined

reduceNode :: BDD -> BDD
reduceNode (1, n) = (1, n) 
reduceNode (0, n) = (0, n)
reduceNode (curNodeIndex, nodes)
 =           
    where curInspect@(index, left, right) = lookUp curNodeIndex nodes  
          (leftTree , ll , lr) = lookUp left nodes 
          (rightTree, rl , rr) = lookUp right nodes
          newleft = if ll == lr then ll else left
          newright = if rl == rr then rl else right
          newNode = (index, newleft, newright)
          newNodes = filter (\(nodeIndex, (index, left, right)) -> left\=right) ++ 
------------------------------------------------------
-- Examples for testing...

b1, b2, b3, b4, b5, b6, b7, b8 :: BExp
b1 = Prim False
b2 = Not (And (IdRef 1) (Or (Prim False) (IdRef 2)))
b3 = And (IdRef 1) (Prim True)
b4 = And (IdRef 7) (Or (IdRef 2) (Not (IdRef 3)))
b5 = Not (And (IdRef 7) (Or (IdRef 2) (Not (IdRef 3))))
b6 = Or (And (IdRef 1) (IdRef 2)) (And (IdRef 3) (IdRef 4))
b7 = Or (Not (IdRef 3)) (Or (IdRef 2) (Not (IdRef 9)))
b8 = Or (IdRef 1) (Not (IdRef 1))

bdd1, bdd2, bdd3, bdd4, bdd5, bdd6, bdd7, bdd8 :: BDD
bdd1 = (0,[])
bdd2 = (2,[(4,(2,1,1)),(5,(2,1,0)),(2,(1,4,5))])
bdd3 = (5,[(5,(1,0,1))])
bdd4 = (2,[(2,(2,4,5)),(4,(3,8,9)),(8,(7,0,1)),(9,(7,0,0)),
           (5,(3,10,11)),(10,(7,0,1)),(11,(7,0,1))])
bdd5 = (3,[(4,(3,8,9)),(3,(2,4,5)),(8,(7,1,0)),(9,(7,1,1)),
           (5,(3,10,11)),(10,(7,1,0)),(11,(7,1,0))])
bdd6 = (2,[(2,(1,4,5)),(4,(2,8,9)),(8,(3,16,17)),(16,(4,0,0)),
           (17,(4,0,1)),(9,(3,18,19)),(18,(4,0,0)),(19,(4,0,1)),
           (5,(2,10,11)),(10,(3,20,21)),(20,(4,0,0)),(21,(4,0,1)),
           (11,(3,22,23)),(22,(4,1,1)),(23,(4,1,1))])
bdd7 = (6,[(6,(2,4,5)),(4,(3,8,9)),(8,(9,1,1)),(9,(9,1,0)),
           (5,(3,10,11)),(10,(9,1,1)),(11,(9,1,1))])
bdd8 = (2,[(2,(1,1,1))])

