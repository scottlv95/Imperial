...
tryToLookUp x d kvs 
  ...
  where res = filter ((==x).fst) kvs

-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp y kvs
--   = map fst (filter ((==y).snd) kvs)
 = [ a|(a,b)<-kvs, b==y ]
...
inferType (App f a) env 
 | t1 == inferType a env = t2 
 | otherwise = TErr
    where TFun t1 t2 = inferType f env 
...
unifyPairs ((TVar v, t):ts) sub 
  | occurs v t = Nothing  
  | otherwise = unifyPairs newts ((v,t):sub)
                where newts = zip (map (applySub [(v,t)].fst) ts) (map (applySub [(v,t)].snd) ts)
...
inferPolyType (Number _) = TInt
inferPolyType (Boolean _) = TBool
inferPolyType (Prim s) = lookUp s primTypes
