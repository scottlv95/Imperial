...
getValue id state = value where (scope, value) = lookUp id state

getLocals :: State -> State
getLocals 
  = filter (\(id,(scope,value)) -> scope == Local)

getGlobals :: State -> State
getGlobals
  = filter (\(id,(scope,value)) -> scope == Global)
...
assignArray (A a) (I i) (I v)
  = A ((i,v) : filter (\(index, _) -> index /= i) a)

updateVar :: (Id, Value) -> State -> State
updateVar (id,value) [] = [(id,(Local, value))]
updateVar (id, value) (x@(i,(scope, v)):xs)
   | id == i = (id,(scope,value)) : xs
   | otherwise = x: updateVar (id,value) xs 
...
applyOp Less (I a) (I b) = if a<b then I 1 else I 0
applyOp Equal (I a) (I b) = if a==b then I 1 else I 0
applyOp Index (A a) (I i) = maybe (I 0) I (lookup i a)

bindArgs :: [Id] -> [Value] -> State
-- Pre: the lists have the same length
bindArgs ids values
  = zip ids (map (Local,) values)
